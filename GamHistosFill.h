  ...
  : fChain(0), isMC(itype), dataset(datasetname), version(versionname)
{
  ...
  is22 = (ds=="2022C" || ds=="2022D" || ds=="2022E" || ds=="2022F" ||
	  ...
	  ds=="2022EEQCD" || TString(ds.c_str()).Contains("2022P8") || TString(ds.c_str()).Contains("Summer22MG") ||
...
GamHistosFill::~GamHistosFill()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t GamHistosFill::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t GamHistosFill::LoadTree(Long64_t entry)
{
...
void GamHistosFill::Init(TTree *tree)
{
