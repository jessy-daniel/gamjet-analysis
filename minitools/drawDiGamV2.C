...
void drawDiGamV2() {
  ...
  // Add Z boson with width = 2.5 (+) 2%*91.2 GeV = 3.1 GeV
  TF1 *f1 = new TF1("f1","[0]*pow(x,[1]+[2]*log(0.01*x))+[3]*TMath::Gaus(x,91.2*[4],3.1*[5])+[6]*TMath::Gaus(x,91.2*[7],3.1*[8])",xmin,xmax);
  //f1->SetParameters(1e5,-5,0);
  //f1->SetParameters(5.2294e1,-1.57839,-3.29205e-1);
  //f1->SetParameters(5.22675e1,-1.57829,-3.29220e-1);
  f1->SetParameters(1.7*5.22675e1,-1.57829,-3.29220e-1,0.79,0.990,0.73,0.10,0.9990,2.09);
  //hmggn->Fit(f1,"QRN");
  hmggn->Fit(f1,"RN");
  ...
  TF1 *f2 = new TF1("f1","[0]*pow(x,[1]+[2]*log(0.01*x))+[3]*TMath::Gaus(x,91.2*[4],3.1*[5])+[6]*TMath::Gaus(x,91.2*[7],3.1*[8])+[9]*TMath::Gaus(x,[10],[11],1)",xmin,xmax);
		    //+[12]*TMath::Gaus(x,125.1,2.5)",xmin,xmax);
  ...
  f2->FixParameter(10,m_eta_t-E_b);
  f2->FixParameter(11,width_eta_t);
  //hmggn->Fit(f2,"QRN");
  hmggn->Fit(f2,"RN");
  ...
  int ndf(0), ndf2(0);
  double xchi2min(170.), xchi2max(1700.);
  for (int i = 1; i != hmggd->GetNbinsX()+1; ++i) {
    ...
    if (sigma!=0) {
      ...
      //if (pt>200. && pt<1000.) {
      if (pt>xchi2min && pt<xchi2max) {
	chi2 += pow((data-fit)/sigma,2); ++ndf;
	chi22 += pow((data-fit2)/sigma,2); ++ndf2;
      }
  ...
  tex->DrawLatex(0.45,0.69,Form("#chi^{2} for %1.0f<m_{#gamma#gamma}<%1.0f GeV",xchi2min,xchi2max));
  tex->SetTextColor(kBlue);
  tex->DrawLatex(0.45,0.64,Form("#chi^{2}(B) = %1.1f / %d",chi2,ndf));
				//f1->GetChisquare(),f1->GetNDF()));
  tex->SetTextColor(kRed);
  tex->DrawLatex(0.45,0.60,Form("#chi^{2}(S+B) = %1.1f / %d",chi22,ndf2));
  ...
  tex->DrawLatex(0.56,0.44,Form("= S/1.33#times10^{-4} = %1.1f#pm%1.1f pb",fabs(f2->GetParameter(9))/1.33e-4,f2->GetParError(9)/1.33e-4));
  ...
  c1->SaveAs("pdf/drawDiGamV2/drawDiGamV2.pdf");
  ...
  c2->SaveAs("pdf/drawDiGamV2/drawDiGamV2_diff.pdf");
  ...
  c3->SaveAs("pdf/drawDiGamV2/drawDiGamV2_AtlasEvents.pdf");
  ...
  c4->SaveAs("pdf/drawDiGamV2/drawDiGamV2_diff2.pdf");
